...
<?php echo file_get_contents("updatevaletuser.html"); ?>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($current_username) && !empty($new_username)) {
        ...
        $check_stmt = $link->prepare("SELECT `valet_user_id` FROM `valet_users` WHERE `username` = ?");
        ...
        if ($check_stmt->num_rows > 0) {
            ...
            $update_stmt = $link->prepare("UPDATE `valet_users` SET `username` = ? WHERE `username` = ?");
