...
<?php echo file_get_contents("updatepw.html"); ?>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($current_password) && !empty($new_password) && !empty($current_username)) {
        ...
        $check_stmt = $link->prepare("SELECT `password` FROM `users` WHERE `username` = ?");
        ...
        if ($check_stmt->num_rows > 0) {
            ...
            $update_stmt = $link->prepare("UPDATE `users` SET `password` = ? WHERE `username` = ?");
